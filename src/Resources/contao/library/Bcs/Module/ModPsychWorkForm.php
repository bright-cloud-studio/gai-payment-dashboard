...
class ModPsychWorkForm extends \Contao\Module
{
    ...
    protected function compile()
    {
        ...
        if($member->pwf_hidden_assignments != null)
            $hidden_assignments = $member->pwf_hidden_assignments;

        $assignments = Assignment::findBy(['psychologist = ?'], [$member->id]);
        foreach($assignments as $assignment) {
            
            if(!in_array($assignment->id, $hidden_assignments)) {
                ...
                // Meeting Time
                if(1 == 1) {
                    $template_assignments[$assignment->id]['meeting_time'] = "<a name='meeting_time_$assignment->id' class='meeting_time' id='meeting_time_$assignment->id' autocomplete='off'><i class='fas fa-edit'></i> $assignment->meeting_time</a>";
